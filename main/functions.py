...
class Functions(object):
    ...
    def insert_people_into_dat(self, filename):
        ...
        with open(filename, 'w') as myFile:
            for person in people:
                ...
                name = ('Unknown' if not person['name'] else str(person['name']).encode("utf-8"))
    ...
    def update(self, USER_ID, baseline, path, pred_items):
        ...
        cPickle.dump(count_dict, open(path + "count_dict.p", "wb"))
        ...
        cPickle.dump(occurrences, open(path + "occurrences.p", "wb"))

        print "Dumping sparse matrix pickle..."
        cPickle.dump(baseline._matrix.get(), open(path + "sparse_matrix.p", "w"))

        print "Updating matrix..."
        baseline.compute(k=100, min_values=None, pre_normalize=None, mean_center=False, post_normalize=True, savefile="../recsys/tmp/model")


    def recommend(self, baseline, path, USER_ID):
        print "Loading sparse matrix pickle..."
        s_matrix = cPickle.load(open(path + "sparse_matrix.p"))

        baseline._matrix.set(matrix=s_matrix)
