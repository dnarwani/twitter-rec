...
class Reconstruction(LabeledMatrixMixin):
    """
    ...
    def set_symmetric_labels(self, labels):
        ...
        only on a symmetric Reconstruction, and it will check to see if
    ...
    def make_symmetric(self):
        """
        Ensure that the Reconstruction is updated symmetrically, with
    ...
    def make_random(rows, cols, k, learning_rate=0.001):
        """
        Generate a random Reconstruction that multiplies an (m x k)
        ...
        return Reconstruction(left, right, learning_rate=learning_rate)
    ...
    def transpose(self):
        return Reconstruction(self.right.T, self.left.T)
    ...
    def copy(self):
        return Reconstruction(self.left, self.right)
    ...
    def __getitem__(self, indices):
        ...
        if (not np.isscalar(indices[0])) and (not np.isscalar(indices[1])):
            ...
            # computations.
            return Reconstruction(leftpart, rightpart, self.shifts)
        else:
    ...
    def __repr__(self):
        return "<Reconstruction: %d by %d>" % (self.shape[0], self.shape[1])
