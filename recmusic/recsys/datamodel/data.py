            ...
            for line in path.splitlines():
                ...
                if not format:
                    ...
                    try:
                        ...
                        row_id = row_id.strip()
                        col_id = col_id.strip()
                        if format.has_key('ids') and (format['ids'] == str or format['ids'] == 'str'):
                            try:
                                row_id = row_id
                            except:
                                print 'Error (ID is not string) while reading: %s' % data #Just ignore that line
                                continue
                            try:
                                col_id = col_id
                            except:
                                print 'Error (ID is not string) while reading: %s' % data #Just ignore that line
                                continue
                        elif format.has_key('ids') and (format['ids'] == float or format['ids'] == 'float'):
                            try:
                                row_id = float(row_id)
                            except:
                                print 'Error (ID is not float) while reading: %s' % data #Just ignore that line
                                continue
                            try:
                                col_id = float(col_id)
                            except:
                                print 'Error (ID is not float) while reading: %s' % data #Just ignore that line
                                continue
                        elif format.has_key('ids') and (format['ids'] == int or format['ids'] == 'int'):
                            try:
                                row_id = int(row_id)
                            except:
